		...
		TString pdfSet, int pdfMember, double muR, double muF, double yieldScale)
{
	...
	else if ( lepSel == EMu ) LeptonID = 13 ;
	...
	//table TrigMu17SF("EfficiencyTables/Efficiency_SF_Mu17Mu8_emu.txt");
	table TrigMu20SF("EfficiencyTables/Efficiency_SF_Mu20.txt");
	table LeptIdEl("EfficiencyTables/electron_new_total.txt");
	...
	// 76X
	table Tight_MuID_76X("EfficiencyTables/2015/MuID_Tight_2015_76X.txt");
	table Loose_MuIso_76X("EfficiencyTables/2015/MuIso_Loose_2015_76X.txt");
	table TrigMu17Mu8SF_76X("EfficiencyTables/2015/DMu_Trig_76X.txt");

	LeptId = Tight_MuID_76X;
	LeptIso = Loose_MuIso_76X;
	LeptIdIso = Iso_TightID13TeV;
	LeptTrig = TrigMu17Mu8SF_76X;
	//Ele_Rec = Ele_Rec_8TeV;
	//if (lepSel == DE || lepSel == SE) LeptId = SC_Ele_2012EA;
	//else if (lepSel == SMu) LeptTrig = TrigIsoMu24SF;
	if (lepSel == EMu) LeptTrig = TrigMu20SF;
	//==========================================================================================================//
	...
	//====================================//
	cout << "Lepton Flavor: " << getLepSel_string(lepSel) << "  systematics: " << systematics << "  direction: " << direction << endl;
	...
	for (Long64_t jentry = entry_start; jentry < entry_stop; jentry += 1) {
		...
		if (nEvents % mess_every_n == 0 && nEvents > mess_every_n){
			timeval t1;
			gettimeofday(&t1, 0);
			double rate = ((t1.tv_sec - t0.tv_sec) + 1.e-6*(t1.tv_usec - t0.tv_usec)) / (nEvents - mess_every_n);
		...
		if(!EvtIsRealData){
			//  xsec_ = 1. ;
			if ( DEBUG ) cout << " TESTING: yieldScale " << yieldScale <<"lumi  " << lumi_ << "  xsec " << xsec_ << " xsecFactor_ " << " skimAccep_ " << skimAccep_[0] << endl;
		...
		if (hasRecoInfo) {
			...
			//--- get E-Muons ---
			if (lepSel == EMu) {
				getElectrons(leptons, vetoMuons);
				if (leptons.size()>0){
					getMuons(leptons, vetoMuons);
				}
				else
			...
			if ((lepSel == DMu || lepSel == DE || lepSel == EMu) && nLeptons >= 2) {
				...
				if (!EvtIsRealData) {
					...
					//}
						...
						if(leptons[0].lepID == 13){  // if muon apply muon's is and ISO sf
							effWeight *= LeptId.getEfficiency(leptons[0].v.Pt(), fabs(leptons[0].v.Eta()));
							effWeight *= LeptIso.getEfficiency(leptons[0].v.Pt(), fabs(leptons[0].v.Eta()));
							effWeight *= LeptIdEl.getEfficiency(leptons[1].v.Pt(), fabs(leptons[1].v.Eta()));  // for electrons only Id is available
						}
						else{
							effWeight *= LeptIdEl.getEfficiency(leptons[0].v.Pt(), fabs(leptons[0].v.Eta()));
							effWeight *= LeptId.getEfficiency(leptons[1].v.Pt(), fabs(leptons[1].v.Eta()));
							effWeight *= LeptIso.getEfficiency(leptons[1].v.Pt(), fabs(leptons[1].v.Eta()));
						}
						if (useTriggerCorrection){
							for(auto &item:leptons){
								if(item.lepID==13){
									effWeight *= LeptTrig.getEfficiency(item.v.Pt(), fabs(item.v.Eta()));
									break;
								}
							}
						}
				...
				if (!EvtIsRealData) {
					...
					if (lepSel == SMu) {
						if(pogSF){
							effWeight *= LeptId.getEfficiency(leptons[0].v.Pt(), fabs(leptons[0].v.Eta()));
							effWeight *= LeptId.getEfficiency(leptons[1].v.Pt(), fabs(leptons[1].v.Eta()));
							// ZASTO ????????			    effWeight *= LeptIso.getEfficiency(leptons[0].v.Pt(), fabs(leptons[0].v.Eta()));
							//			    effWeight *= LeptIso.getEfficiency(leptons[1].v.Pt(), fabs(leptons[1].v.Eta()));
						} else{
							effWeight *= LeptIdIso.getEfficiency(leptons[0].v.Pt(), fabs(leptons[0].v.Eta()));
						}
		...
		if (hasRecoInfo){     
			...
			nGoodBJets_b=0; nGoodBJets_l=0;nGoodBJets_c=0;
			for (unsigned short i(0); i < nGoodJets; i++) {
				if ( jets[i].isBJet ) {
					nGoodBJets++;
		...
		if (hasGenInfo){
			if (passesgenLeptonCut){
				...
				/*
				   if(UnfoldUnc){
				   int binNumber = ZNGoodJets_Zexc_ratio->GetXaxis()->FindBin(nGoodJets);
				...
				if (nGoodGenJets >= 2) {
					...
					/*     if(nGoodJets >= 2 && UnfoldUnc){
						   double binNumber = SecondJetEta_2_Zinc2jet_ratio->GetXaxis()->FindBin(fabs(jets[1].v.Eta()));
						   RatioValue =  SecondJetEta_2_Zinc2jet_ratio->GetBinContent(binNumber);
						   double binNumber1 = JetsHT_2_Zinc2jet_ratio->GetXaxis()->FindBin(jetsHT);
					...
					genJetsHT_Zinc2jet->Fill(genJetsHT, genWeight*RatioValue);  /// in original file we have here RatioValue1 
				...
				if (nGoodGenJets_20 >= 3) {
					...
					/*       if(nGoodJets_20 >= 3 && UnfoldUnc){
							 double binNumber = ThirdJetPt_2_Zinc3jet_ratio->GetXaxis()->FindBin(jets_20[2].v.Pt());
				...
				if (nGoodGenJets >= 3){
					...
					/*   if(nGoodJets >= 3 && UnfoldUnc){
						 ...
						 double binNumber1 = JetsHT_2_Zinc3jet_ratio->GetXaxis()->FindBin(jetsHT);
		...
		if (hasRecoInfo && passesLeptonCut && (!bTagJetFound || !rejectBTagEvents)) { 
			...
			if (nGoodJets_20 >= 1) {
				...
				double RatioValue=1;
				/*		if(UnfoldUnc){
						double binNumber = FirstJetPt_2_Zinc1jet_ratio->GetXaxis()->FindBin(jets_20[0].v.Pt());
			...
			if (nGoodJets >= 1){
				...
				if (DEBUG) cout << __LINE__ << "  aaaa1 " << EWKBoson.Pt() << "  " << leptons.size() << endl;
				...
				if (DEBUG) cout << "Eta: " << leptons[0].v.Eta() << endl;
				...
				//JetsHT_Zinc1jet->Fill(jetsHT, weight*RatioValue); /// in original file we have here RatioValue1 
				if (DEBUG) cout << __LINE__ << "  aaaa1 " << EWKBoson.Pt() << "  " << dRZJetMin << endl;
			...
			if (nGoodBJets>=1){
				...
				ZMassBJets_Zinc1Bjet->Fill(EWKBoson.M(),weightBJets);
				HtBJets_Zinc1Bjet->Fill(jetsHT,weightBJets);
				for (auto &jet:jets){
					if (jet.isBJet){
						FirstBJetPt_Zinc1Bjet->Fill(jet.v.Pt(),weightBJets);
						mindRBJetToZ_Zinc1Bjet->Fill(deltaR(EWKBoson, jet.v),weightBJets);
						mindPhiBJetToZ_Zinc1Bjet->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
						break;
					}
				}
				if (evFlav==0){
					ZptBJets_Zinc1Bjet_l->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zinc1Bjet_l->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zinc1Bjet_l->Fill(jetsHT,weightBJets);
					for (auto &jet:jets){
						if (jet.isBJet){
							FirstBJetPt_Zinc1Bjet_l->Fill(jet.v.Pt(),weightBJets);
							mindRBJetToZ_Zinc1Bjet_l->Fill(deltaR(EWKBoson, jet.v),weightBJets);
							mindPhiBJetToZ_Zinc1Bjet_l->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
							break;
						}
					}
				}
				if (evFlav==4){
					ZptBJets_Zinc1Bjet_c->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zinc1Bjet_c->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zinc1Bjet_c->Fill(jetsHT,weightBJets);
					for (auto &jet:jets){
						if (jet.isBJet){
							FirstBJetPt_Zinc1Bjet_c->Fill(jet.v.Pt(),weightBJets);
							mindRBJetToZ_Zinc1Bjet_c->Fill(deltaR(EWKBoson, jet.v),weightBJets);
							mindPhiBJetToZ_Zinc1Bjet_c->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
							break;
						}
					}
				}
				if (evFlav==5){
					ZptBJets_Zinc1Bjet_b->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zinc1Bjet_b->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zinc1Bjet_b->Fill(jetsHT,weightBJets);
					for (auto &jet:jets){
						if (jet.isBJet){
							FirstBJetPt_Zinc1Bjet_b->Fill(jet.v.Pt(),weightBJets);
							mindRBJetToZ_Zinc1Bjet_b->Fill(deltaR(EWKBoson, jet.v),weightBJets);
							mindPhiBJetToZ_Zinc1Bjet_b->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
							break;
						}
					}
				}
			...
			if (nGoodBJets>=2){
				...
				ZMassBJets_Zinc2Bjet->Fill(EWKBoson.M(),weightBJets);
				HtBJets_Zinc2Bjet->Fill(jetsHT,weightBJets);
				if (evFlav==5) {
					ZptBJets_Zinc2Bjet_b->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zinc2Bjet_b->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zinc2Bjet_b->Fill(jetsHT,weightBJets);
				}
				else if (evFlav==4) {
					ZptBJets_Zinc2Bjet_c->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zinc2Bjet_c->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zinc2Bjet_c->Fill(jetsHT,weightBJets);
				}
				else {
					ZptBJets_Zinc2Bjet_l->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zinc2Bjet_l->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zinc2Bjet_l->Fill(jetsHT,weightBJets);
				}
				...
				for (auto &jet:jets){
					if (jet.isBJet && cnt==0){
						...
						SecondBJetPt_Zinc2Bjet->Fill(jet.v.Pt(), weightBJets);
						if (evFlav==5) SecondBJetPt_Zinc2Bjet_b->Fill(jet.v.Pt(), weightBJets);
						else if (evFlav==4) SecondBJetPt_Zinc2Bjet_c->Fill(jet.v.Pt(), weightBJets);
						else SecondBJetPt_Zinc2Bjet_l->Fill(jet.v.Pt(), weightBJets);
						Jet2 = jet.v;
						break;
					}
				}
				...
				mindRBJetToZ_Zinc2Bjet->Fill(dR1,weightBJets);
				maxdRBJetToZ_Zinc2Bjet->Fill(dR2,weightBJets);
				mindPhiBJetToZ_Zinc2Bjet->Fill(dPhi1,weightBJets);
				if (evFlav==5) {
					BJetsMass_Zinc2Bjet_b->Fill((Jet1+Jet2).M(), weightBJets);
			...
			if (nGoodBJets>=3){
				METE_Zinc3Bjet->Fill(MET.E(),weightBJets);
				METPt_Zinc3Bjet->Fill(MET.Pt(),weightBJets);
				ZptBJets_Zinc3Bjet->Fill(EWKBoson.Pt(),weightBJets);
				if (evFlav==5) ZptBJets_Zinc3Bjet_b->Fill(EWKBoson.Pt(), weightBJets);
				else if (evFlav==4) ZptBJets_Zinc3Bjet_c->Fill(EWKBoson.Pt(), weightBJets);
			...
			if (nGoodBJets==1){
				//METE_Zinc1Bjet->Fill(MET.E(),weightBJets);
				//METPt_Zinc1Bjet->Fill(MET.Pt(),weightBJets);
				ZptBJets_Zexc1Bjet->Fill(EWKBoson.Pt(),weightBJets);
				ZMassBJets_Zexc1Bjet->Fill(EWKBoson.M(),weightBJets);
				HtBJets_Zexc1Bjet->Fill(jetsHT,weightBJets);
				for (auto &jet:jets){
					if (jet.isBJet){
						FirstBJetPt_Zexc1Bjet->Fill(jet.v.Pt(),weightBJets);
						mindRBJetToZ_Zexc1Bjet->Fill(deltaR(EWKBoson, jet.v),weightBJets);
						mindPhiBJetToZ_Zexc1Bjet->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
						break;
					}
				}
				if (evFlav==0){
					ZptBJets_Zexc1Bjet_l->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zexc1Bjet_l->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zexc1Bjet_l->Fill(jetsHT,weightBJets);
					for (auto &jet:jets){
						if (jet.isBJet){
							FirstBJetPt_Zexc1Bjet_l->Fill(jet.v.Pt(),weightBJets);
							mindRBJetToZ_Zexc1Bjet_l->Fill(deltaR(EWKBoson, jet.v),weightBJets);
							mindPhiBJetToZ_Zexc1Bjet_l->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
							break;
						}
					}
				}
				if (evFlav==4){
					ZptBJets_Zexc1Bjet_c->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zexc1Bjet_c->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zexc1Bjet_c->Fill(jetsHT,weightBJets);
					for (auto &jet:jets){
						if (jet.isBJet){
							FirstBJetPt_Zexc1Bjet_c->Fill(jet.v.Pt(),weightBJets);
							mindRBJetToZ_Zexc1Bjet_c->Fill(deltaR(EWKBoson, jet.v),weightBJets);
							mindPhiBJetToZ_Zexc1Bjet_c->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
							break;
						}
					}
				}
				if (evFlav==5){
					ZptBJets_Zexc1Bjet_b->Fill(EWKBoson.Pt(),weightBJets);
					ZMassBJets_Zexc1Bjet_b->Fill(EWKBoson.M(),weightBJets);
					HtBJets_Zexc1Bjet_b->Fill(jetsHT,weightBJets);
					for (auto &jet:jets){
						if (jet.isBJet){
							FirstBJetPt_Zexc1Bjet_b->Fill(jet.v.Pt(),weightBJets);
							mindRBJetToZ_Zexc1Bjet_b->Fill(deltaR(EWKBoson, jet.v),weightBJets);
							mindPhiBJetToZ_Zexc1Bjet_b->Fill(deltaPhi(EWKBoson, jet.v),weightBJets);
							break;
						}
					}
				}
			...
			// 2 b jets - exc
			if (nGoodBJets==2){
				//METE_Zexc2Bjet->Fill(MET.E(),weightBJets);
				//METPt_Zexc2Bjet->Fill(MET.Pt(),weightBJets);
				ZptBJets_Zexc2Bjet->Fill(EWKBoson.Pt(),weightBJets);
				HtBJets_Zexc2Bjet->Fill(jetsHT,weightBJets);
				if (evFlav==5) {
					ZptBJets_Zexc2Bjet_b->Fill(EWKBoson.Pt(),weightBJets);
					HtBJets_Zexc2Bjet_b->Fill(jetsHT,weightBJets);
				}
				else if (evFlav==4) {
					ZptBJets_Zexc2Bjet_c->Fill(EWKBoson.Pt(),weightBJets);
					HtBJets_Zexc2Bjet_c->Fill(jetsHT,weightBJets);
				}
				else {
					ZptBJets_Zexc2Bjet_l->Fill(EWKBoson.Pt(),weightBJets);
					HtBJets_Zexc2Bjet_l->Fill(jetsHT,weightBJets);
				}
				...
				for (auto &jet:jets){
					if (jet.isBJet && cnt==0){
						FirstBJetPt_Zexc2Bjet->Fill(jet.v.Pt(), weightBJets);
						if (evFlav==5) FirstBJetPt_Zexc2Bjet_b->Fill(jet.v.Pt(), weightBJets);
						else if (evFlav==4) FirstBJetPt_Zexc2Bjet_c->Fill(jet.v.Pt(), weightBJets);
						else FirstBJetPt_Zexc2Bjet_l->Fill(jet.v.Pt(), weightBJets);
						Jet1 = jet.v;
						cnt++;
					}
					else if (jet.isBJet && cnt==1){
						SecondBJetPt_Zexc2Bjet->Fill(jet.v.Pt(), weightBJets);
						if (evFlav==5) SecondBJetPt_Zexc2Bjet_b->Fill(jet.v.Pt(), weightBJets);
						else if (evFlav==4) SecondBJetPt_Zexc2Bjet_c->Fill(jet.v.Pt(), weightBJets);
						else SecondBJetPt_Zexc2Bjet_l->Fill(jet.v.Pt(), weightBJets);
						Jet2 = jet.v;
						break;
					}
				}
				BJetsMass_Zexc2Bjet->Fill((Jet1+Jet2).M(), weightBJets);
				double dR1 = deltaR(EWKBoson,Jet1);
				double dR2 = deltaR(EWKBoson,Jet2);
				double dPhi1 = deltaPhi(EWKBoson,Jet1);
				double dPhi2 = deltaPhi(EWKBoson,Jet2);
				mindRBJets_Zexc2Bjet->Fill(deltaR(Jet1, Jet2), weightBJets);
				mindPhiBJets_Zexc2Bjet->Fill(deltaPhi(Jet1, Jet2), weightBJets);
				if (dR1>dR2) {
					double tmp = dR2;
					dR2=dR1;
					dR1=tmp;
				...
				mindRBJetToZ_Zexc2Bjet->Fill(dR1,weightBJets);
				maxdRBJetToZ_Zexc2Bjet->Fill(dR2,weightBJets);
				mindPhiBJetToZ_Zexc2Bjet->Fill(dPhi1,weightBJets);
				if (evFlav==5) {
					BJetsMass_Zexc2Bjet_b->Fill((Jet1+Jet2).M(), weightBJets);
...
void ZJets::getEMuons(vector<leptonStruct>& leptons,  vector<leptonStruct>& vetoLeptons)
{
	...
	for (unsigned short i(0); i < nTotMu; i++) {
		leptonStruct mu(MuPt->at(i),
				...
				MuIdTight->at(i),  
				MuPfIso->at(i), 
				MuEta->at(i),    /// SC Eta -- if one needs it !!!
				0,	
				13);  // CommentAG
		...
		else if (lepSel == EMu && muPassesVetoPtCut && muPassesVetoEtaCut) {  //CommentAG:  need to check muPassesVetoIdCut! 
			vetoLeptons.push_back(mu);
		}
	...
	for (unsigned short i(0); i < nTotEl; i++){
		...
		leptonStruct ele(ElPt->at(i), 
				...
				ElEtaSc->at(i), 
				0., 
				11); // CommentAG patElecTrig_->at(i)
		...
		// select the veto electrons
			...
			vetoLeptons.push_back(ele);   // TO DO
		}
...
void ZJets::getMuons(vector<leptonStruct>& leptons,  vector<leptonStruct>& vetoMuons)
{
	...
	if ( DEBUG ) cout << " getMuons" << __LINE__ << endl;
...
void ZJets::getElectrons(vector<leptonStruct>& leptons,  vector<leptonStruct>& vetoElectrons)
{
	...
	ZJets::ZJets(selection lepSel_, TString sampleLabel, TString fileName_,
		...
		HistoSetZJets(getLepSel_string(lepSel_).Data()), outputDirectory(outDir_),
		...
		triggerMask_(0), triggerMaskSet_(false), muIso_(0), eIso_(0)
	{
		...
		rejectBTagEvents = getLepSel_string(lepSel).BeginsWith("S"); 
	...
	void ZJets::CreateOutputFileName(const TString& pdfSet, int pdfMember, double muR, double muF, int iJob){
		outputFileName =  CreateOutputFileName(pdfSet, pdfMember, muR, muF, iJob,
				getLepSel_string(lepSel), sampleLabel_, useTriggerCorrection, systematics, direction,
	...
	void ZJets::Init(bool hasRecoInfo, bool hasGenInfo){
		...
		if (hasRecoInfo){
			...
			if ( DEBUG ) cout << __LINE__ <<" initialise branches" << endl;
		}
		if (hasGenInfo){
	...
	bool ZJets::setTriggerMask(){
		...
		else if(lepSel == EMu){ branchName = "TrigHltMu"; ourTrig_ = &TrigHltMu; }
		else{
			std::cerr << __FILE__ << ":" << __LINE__ << ". "
				<< "lepSel value, '" << getLepSel_string(lepSel)
